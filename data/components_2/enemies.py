...
class Enemy(pg.sprite.Sprite):
    ...
    def walking(self):
        ...
        if (self.current_time - self.animate_timer) > 125:
            if self.direction == c.LEFT and (self.frame_index == 3 or self.frame_index == 4):
                self.frame_index = 0
            if self.frame_index == 0 and self.direction == c.LEFT:
                self.frame_index += 1
            elif self.frame_index == 1 and self.direction == c.LEFT:
                self.frame_index = 0
            elif self.direction == c.RIGHT and (self.frame_index == 0 or self.frame_index == 1):
                self.frame_index = 3
            if self.frame_index == 3 and self.direction == c.RIGHT:
                self.frame_index += 1
            elif self.frame_index == 4 and self.direction == c.RIGHT:
                self.frame_index = 3
...
class Goomba(Enemy):
    ...
    def setup_frames(self):
        ...
        self.frames.append(
            self.get_image(0, 34, 16, 16))
        self.frames.append(
            self.get_image(30, 34, 16, 16))
        self.frames.append(
            self.get_image(61, 30, 16, 16))
...
class Koopa(Enemy):
    ...
    def setup_frames(self):
        """Sets frame list"""
        self.frames.append(
            self.get_image(150, 60, 16, 24))
        self.frames.append(
            self.get_image(180, 60, 16, 24))
        self.frames.append(
            self.get_image(360, 65, 16, 15))
        self.frames.append(pg.transform.flip(self.frames[0], True, False))
        self.frames.append(pg.transform.flip(self.frames[1], True, False))
...
# class Piranha(Enemy):
#
#     def __init__(self, y=c.GROUND_HEIGHT, x=0, direction=c.UP, name='piranha'):
#         Enemy.__init__(self)
#         Enemy.state = C.JUMP
#         self.setup_enemy(x, y, direction, name, self.setup_frames)
#
